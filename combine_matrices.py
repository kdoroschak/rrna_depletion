...
def combine_matrices(indir, outdir):
	...
	if os.path.exists(outdir + "/unique_genes.npy") \
	   ...
	   genes = np.load(outdir + "/unique_genes.npy")
	   gene_files = np.load(outdir + "/unique_genes_matrix_filenames.npy").item()
		...
		for matrix_file in matrix_files:
			print matrix_file
			gene = matrix_file.split("_")[-3]
			print gene
			genes.add(gene)
			try:
				gene_files[gene].append(indir + "/" + matrix_file)
			except:
	...
	for gene in genes:
		...
		process_gene(gene, files, outdir, indir=indir)
	print "Done..."
		

def process_gene(gene_name, gene_matrix_full_filenames, outdir, indir=None):
	...
	for matrix_file in gene_matrix_full_filenames:
		...
		try:
			...
			try:
				matrix = np.load(indir + "/" + matrix_file)
			except:
				print matrix_file
				return
